...
import { X, CheckCircle, Zap, Target, Clock, Share2, Trophy, TrendingUp, BrainCircuit, Play, Sparkles } from 'lucide-react-native';
...
const GridCell = React.memo(({ value, target, lastTapCorrect, onPress, cellSize }: CellProps) => {
    ...
    const animatedStyles = useAnimatedStyle(() => {
        return {
            backgroundColor: pressed.value ? Colors.mentra.brandPrimary : Colors.mentra.surface,
            ...
            <Animated.View
                style={[
                    ...
                    animatedStyles,
                    {
                        width: cellSize,
                        height: cellSize,
                        borderColor: isTarget ? Colors.mentra.brandPrimary : Colors.mentra.border,
                    },
                ]}
            >
...
export default function GridFocusScreen() {
    ...
    if (gameState === 'idle' || gameState === 'pre_game') {
        return (
            <View style={[styles.container, { paddingTop: insets.top + 20, paddingHorizontal: 24, justifyContent: 'center' }]}>
    ...
    const timerColor = timePercent > 0.4
        ...
        : timePercent > 0.2
            ? Colors.mentra.warning
            : Colors.mentra.danger;
        ...
        <GestureHandlerRootView style={{ flex: 1 }}>
            <View style={[styles.container, { paddingTop: insets.top }]}>
                ...
                {lastTapCorrect === false && (
                    <Animated.View entering={FadeIn} exiting={FadeOut} style={styles.wrongFlash} />
                )}
            </View>
...
const styles = StyleSheet.create({
    ...
    // Countdown
    countdownText: { fontSize: 96, fontWeight: '800', color: Colors.mentra.brandPrimary, textAlign: 'center' },
    countdownLabel: { fontSize: 16, color: Colors.mentra.textDim, marginTop: 12, textAlign: 'center' },
    ...
    closeBtn: {
        padding: 10, borderRadius: 24, backgroundColor: Colors.mentra.surface,
        borderWidth: 1.5, borderColor: Colors.mentra.border,
        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2
    },
    headerCenter: { alignItems: 'center', flex: 1 },
    headerLabel: { fontSize: 13, color: Colors.mentra.textDim, fontWeight: '700', letterSpacing: 2, textTransform: 'uppercase' },
    targetNumber: { fontSize: 48, fontWeight: '900', color: Colors.mentra.text, letterSpacing: -1 },
    timerBox: {
        flexDirection: 'row', alignItems: 'center', gap: 6,
        backgroundColor: Colors.mentra.surface, borderRadius: 24,
        paddingHorizontal: 16, paddingVertical: 10,
        borderWidth: 1.5, borderColor: Colors.mentra.border,
        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2
    },
    timerBoxUrgent: { backgroundColor: Colors.mentra.danger, borderColor: Colors.mentra.danger },
    timerText: { fontSize: 16, fontWeight: '800' },

    // Progress
    progressBg: { height: 6, backgroundColor: Colors.mentra.surface2, marginHorizontal: 24, borderRadius: 6, overflow: 'hidden' },
    progressFill: { height: 6, borderRadius: 6 },

    // Grid
    gridContainer: { flex: 1, alignItems: 'center', justifyContent: 'center' },
    cell: {
        borderRadius: 16, borderWidth: 1.5, borderColor: Colors.mentra.border,
    ...
    cellText: { fontSize: 28, fontWeight: '800', color: Colors.mentra.text },
    cellTextTarget: { color: Colors.mentra.brandPrimary },
    ...
    resultsTitle: { fontSize: 11, fontWeight: '800', color: Colors.mentra.textDim, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 4 },
    ...
    fpqLabel: { fontSize: 13, color: Colors.mentra.textDim, marginBottom: 16 },
    ...
    prevBestText: { fontSize: 13, color: Colors.mentra.textDim },

    // Stats
    statsRow: { flexDirection: 'row', gap: 10, marginBottom: 24, width: '100%' },
    statCard: {
        flex: 1, backgroundColor: Colors.mentra.surface, borderRadius: 16,
        paddingVertical: 14, paddingHorizontal: 4, alignItems: 'center', gap: 5,
        borderWidth: 1.5, borderColor: Colors.mentra.border,
        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2
    },
    statValue: { fontSize: 18, fontWeight: '800', color: Colors.mentra.text },
    statLabel: { fontSize: 11, color: Colors.mentra.textDim, textTransform: 'uppercase', letterSpacing: 0.5 },

    // CTAs
    retryBtn: { width: '100%', backgroundColor: Colors.mentra.brandPrimary, borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginBottom: 10 },
    retryBtnText: { color: '#FFF', fontSize: 16, fontWeight: '700' },
    shareBtn: { width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 14, paddingVertical: 14, marginBottom: 4, borderWidth: 1.5, borderColor: Colors.mentra.brandPrimary, backgroundColor: Colors.mentra.brandPrimary + '10' },
    shareBtnText: { color: Colors.mentra.brandPrimary, fontSize: 15, fontWeight: '700' },
    backBtn: { width: '100%', borderRadius: 14, paddingVertical: 12, alignItems: 'center' },
    backBtnText: { color: Colors.mentra.textDim, fontSize: 14, fontWeight: '600' },

    // Intro Screen (Pre_Game)
    introIconBox: { width: 72, height: 72, borderRadius: 24, backgroundColor: Colors.mentra.brandPrimary + '15', alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
    introTitle: { fontSize: 28, fontWeight: '800', color: Colors.mentra.text, marginBottom: 8, textAlign: 'center' },
    introSub: { fontSize: 15, color: Colors.mentra.textDim, fontWeight: '600', letterSpacing: 1, textTransform: 'uppercase', textAlign: 'center' },
    instructionsBox: {
        backgroundColor: Colors.mentra.surface,
    ...
    introStartBtn: { width: '100%', backgroundColor: Colors.mentra.brandPrimary, borderRadius: 16, paddingVertical: 18, alignItems: 'center', shadowColor: Colors.mentra.brandPrimary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 6 },
